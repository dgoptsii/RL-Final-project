...
LOG_STD_MIN = -20
LOG_STD_MAX = 2
DEFAULT_ALGORITHM_NAME = "sac_prioritized_her"
...
def load_actor(checkpoint_path, device):
    ...
    required = ["obs_dim", "act_dim", "hidden_dim", "act_limit", "actor_state_dict"]
    missing = [key for key in required if key not in checkpoint]
    if missing:
        raise KeyError(
            f"Checkpoint {checkpoint_path} is missing keys {missing}. "
            "Make sure it was saved with save_checkpoint(...), not only actor_state_dict."
        )
...
def add_text(frame, text):
    ...
    draw.rectangle((5, 5, 760, 40), fill=(255, 255, 255))
    draw.text((12, 13), text, fill=(0, 0, 0))
...
def rollout(env, actor, device, episode_num, algorithm_name, max_steps=50):
    ...
    for _ in range(max_steps):
        ...
        final_distance = float(np.linalg.norm(obs_dict["achieved_goal"] - obs_dict["desired_goal"]))

        label = (
            f"{algorithm_name.upper()} | checkpoint ep {episode_num} | "
            f"success={final_success:.0f} | return={total_return:.1f} | dist={final_distance:.3f}"
        )
        frame = env.render()
...
def collect_checkpoints(checkpoint_dir):
    ...
    for filename in os.listdir(checkpoint_dir):
        if filename.endswith(".pt") and "episode" in filename:
            episode = extract_episode(filename)
            if episode is not None:
                checkpoint_files.append((episode, filename))
...
def main():
    parser = argparse.ArgumentParser(description="Animate SAC + prioritized HER learning from saved episode checkpoints.")
    ...
    parser.add_argument("--out", type=str, default="results/animations/sac_prioritized_her_learning.gif")
    parser.add_argument("--fps", type=int, default=20)
    parser.add_argument("--max-checkpoints", type=int, default=20)
    parser.add_argument("--max-steps", type=int, default=50)
    parser.add_argument("--success-streak-stop", type=int, default=1)
    ...
    for episode, filename in selected:
        print(f"episode {episode}: {filename}")

    env = gym.make(f"{args.task}-v4", reward_type=args.reward_type, render_mode="rgb_array")

    all_frames = []
    success_streak = 0
    algorithm_name = DEFAULT_ALGORITHM_NAME

    for episode_num, filename in selected:
        ...
        frames, result = rollout(env, actor, device, episode_num, algorithm_name, args.max_steps)
        ...
        print(
            ...
            f"success={result['success']:.0f} | return={result['return']:.2f} | "
            f"distance={result['distance']:.4f} | steps={result['steps']}"
        )
        ...
        if success_streak >= args.success_streak_stop:
            print(f"\nStopping animation after first/streak successful checkpoint: {success_streak}.")
            break
