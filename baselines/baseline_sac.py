...
def main():
    ...
    run_dirs = create_run_dirs("results", "sac", args.task, args.reward_type, args.seed)
    run_name = f"sac_{args.task}_{args.reward_type}_seed{args.seed}"
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for episode in range(1, args.episodes + 1):
            ...
            f.flush()
            # ================= CHECKPOINT SAVING =================
            if args.save_checkpoint_every > 0 and episode % args.save_checkpoint_every == 0:
                checkpoint_path = os.path.join(
                    run_dirs["models"],
                    f"{run_name}_episode{episode}.pt"
                )
    ...
    env.close()
    save_checkpoint(agent, model_path, "sac", args, obs_dim, act_dim, act_limit)
    summary = summarize_run(csv_path, "sac", args.task, args.reward_type, args.seed, args, model_path)
